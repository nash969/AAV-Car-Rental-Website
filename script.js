...
const panels = {
  customer: {
    dashboard: () => `${metrics([["Active Bookings", "2"], ["Pending Payments", "1"], ["Completed Rentals", "8"]])}
...
<select id="rentalDuration"><option selected disabled>Select Rental Duration</option><option>12 Hours</option><option>24 Hours</option><option>3 Days</option><option>1 Week</option><option>2 Weeks</option><option>1 Month</option></select></label><label>Total Cost<input value="₱14,000"></label><button class="primary wide" type="button">Submit Booking</button></div></section>${bookingSummary()}</div>${table("Booking Status", ["Vehicle", "Dates", "Status"], [["Toyota Rush", "Jul 16 - Jul 20", "<span class='status reserved'>Pending</span>"], ["Toyota Vios", "Jun 02 - Jun 04", "<span class='status available'>Completed</span>"]])}`,
